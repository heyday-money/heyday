...
export function InstallmentDialog({ data, plan, onClose }: { data: FinancialData; plan?: Installment; onClose: () => void }) {
  ...
  const [interest, setInterest] = useState(plan ? plan.interest_rate_bps == null ? '' : interestRateText(plan.interest_rate_bps) : '0')
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const interestRate = interest.trim() ? decimalToInteger(interest, 2) : null
      ...
      const input = { purchase, interest_rate_bps: interestRate, id: plan?.id ?? null, name: String(form.get('name')).trim(), account_id: String(form.get('account')), debt_account_id: cardId, monthly_amount: decimalToInteger(amount, digits), installment_count: Number(count), first_due_date: date, currency }
      ...
      <form onSubmit={save} onChange={() => setDirty(true)} className="flex min-h-0 flex-1 flex-col">
        <div className="min-h-0 overflow-y-auto px-6 py-5">
          <fieldset disabled={saving} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <Field label="Annual interest rate (%, optional)"><Input className="mt-2" inputMode="decimal" placeholder="e.g. 0 or 7.5" value={interest} onChange={event => setInterest(event.target.value)} /></Field>
